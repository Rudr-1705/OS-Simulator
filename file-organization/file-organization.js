...
// ── Model Descriptions ───────────────────────────────────────────
const MODEL_DESC = {
  single: "One flat directory; all files coexist. File names must be globally unique.",
...
function makeNode(name, type, extra = {}) {
  return { id: nextId++, name, type, refCount: 0, x: 320, y: 210, vx: 0, vy: 0, highlight: 0, ...extra };
}

function findNode(id) { return fsState.nodes.find(n => n.id === id); }
...
function removeEdge(fromId, toId) {
  fsState.edges = fsState.edges.filter(e => !(e.from === fromId && e.to === toId));
}

function childrenOf(nodeId) {
  return fsState.edges.filter(e => e.from === nodeId).map(e => findNode(e.to)).filter(Boolean);
}

function parentsOf(nodeId) {
  return fsState.edges.filter(e => e.to === nodeId).map(e => findNode(e.from)).filter(Boolean);
}
  ...
  if (model === "single") {
    ...
    users.forEach((u, ui) => {
      ...
      u.files.forEach((fname, fi) => {
        const f = makeNode(fname, TYPE.FILE, { x: 80 + ui * 320 + fi * 130, y: 330 });
        fsState.nodes.push(f);
        addEdge(ud.id, f.id);
      });
    });
    ...
    const shared = makeNode("shared_file", TYPE.FILE, { x: 320, y: 340, refCount: 2 });
...
function updateSidebar() {
  const dirs  = fsState.nodes.filter(n => [TYPE.ROOT, TYPE.DIR, TYPE.USER].includes(n.type)).length;
  const files = fsState.nodes.filter(n => n.type === TYPE.FILE).length;
  // hard links = files pointed to by >1 parent
  const shared = fsState.nodes.filter(n => n.type === TYPE.FILE && parentsOf(n.id).length > 1);
  ...
  nl.innerHTML = fsState.nodes.map(n => {
    ...
    const icon = n.type === TYPE.FILE ? "📄" : (n.type === TYPE.ROOT ? "🗂" : "📁");
...
function updateLegend() {
  ...
  if (currentModel === "dag") items.push({ color: "#a855f7", label: "Shared File (multi-parent)" });
...
function updateModelDesc() {
  document.getElementById("modelDesc").textContent = MODEL_DESC[currentModel] || "";
}
...
function drawNode(n) {
  ...
  // Icon
  const icon = n.type === TYPE.FILE ? "📄" : (n.type === TYPE.ROOT ? "🗂" : n.type === TYPE.USER ? "👤" : "📁");
  ctx.font = "16px serif";
  ctx.textAlign = "center";
  ...
  ctx.fillText(n.name.length > 12 ? n.name.slice(0, 11) + "…" : n.name, n.x, n.y + r + 4);
...
function nodeAt(cx, cy) {
  return fsState.nodes.slice().reverse().find(n => Math.hypot(n.x - cx, n.y - cy) < 32);
}
...
function opSingleAdd() {
  ...
  if (fsState.nodes.find(n => n.type === TYPE.FILE && n.name === name))
    return showToast("File name already exists (names must be unique in single-level)", "error");
  const root = fsState.nodes.find(n => n.type === TYPE.ROOT);
  const count = fsState.nodes.filter(n => n.type === TYPE.FILE).length;
  const f = makeNode(name, TYPE.FILE, { x: 80 + (count % 6) * 100, y: 220 + Math.floor(count / 6) * 100 });
  fsState.nodes.push(f);
  addEdge(root.id, f.id);
  f.highlight = 1;
...
function opTwoAddUser() {
  ...
  u.highlight = 1;
  logTrace(`Created user directory "${name}" under Master Directory.`, "success");
...
function opTwoAddFile() {
  ...
  if (!userName || !fileName) return showToast("Fill in both user and file name", "error");
  const u = fsState.nodes.find(n => n.type === TYPE.USER && n.name === userName);
  ...
  if (siblings.includes(fileName)) return showToast(`File "${fileName}" already in ${userName}'s directory`, "error");
  const uFiles = childrenOf(u.id).length;
  const f = makeNode(fileName, TYPE.FILE, { x: u.x - 60 + uFiles * 80, y: u.y + 140 });
  fsState.nodes.push(f);
  addEdge(u.id, f.id);
  f.highlight = 1;
  logTrace(`Created file "${fileName}" in user directory "${userName}".`, "success");
...
function opTreeAddDir() {
  ...
  if (!parentName || !dirName) return showToast("Fill in parent and new directory name", "error");
  const parent = fsState.nodes.find(n => [TYPE.ROOT, TYPE.DIR].includes(n.type) && n.name === parentName);
  if (!parent) return showToast(`Directory "${parentName}" not found`, "error");
  const d = makeNode(dirName, TYPE.DIR, { x: parent.x + (Math.random() - 0.5) * 200, y: parent.y + 120 });
...
function opTreeAddFile() {
  ...
  if (!parentName || !fileName) return showToast("Fill in parent directory and file name", "error");
  const parent = fsState.nodes.find(n => [TYPE.ROOT, TYPE.DIR].includes(n.type) && n.name === parentName);
  if (!parent) return showToast(`Directory "${parentName}" not found`, "error");
  const f = makeNode(fileName, TYPE.FILE, { x: parent.x + (Math.random() - 0.5) * 160, y: parent.y + 110 });
  fsState.nodes.push(f);
...
function opDagCreateDir() {
  ...
  if (fsState.nodes.find(n => n.name === name)) return showToast("Name already exists", "error");
  const root = fsState.nodes.find(n => n.type === TYPE.ROOT);
...
function opDagCreateFile() {
  ...
  if (!dirName || !fileName) return showToast("Fill in directory and file name", "error");
  const dir = fsState.nodes.find(n => [TYPE.DIR, TYPE.ROOT].includes(n.type) && n.name === dirName);
  if (!dir) return showToast(`Directory "${dirName}" not found`, "error");
  if (fsState.nodes.find(n => n.type === TYPE.FILE && n.name === fileName))
    return showToast(`File "${fileName}" already exists — use Share to link it to more directories`, "info");
  const f = makeNode(fileName, TYPE.FILE, { x: dir.x + (Math.random() - 0.5) * 120, y: dir.y + 150 });
  fsState.nodes.push(f);
  addEdge(dir.id, f.id, "link");
  f.highlight = 1;
  logTrace(`Created file "${fileName}" in "${dirName}". Reference count = 1.`, "success");
  showToast(`File "${fileName}" created`, "success");
  updateSidebar();
...
function opDagShare() {
  ...
  if (!fileName || !dirName) return showToast("Fill in file and target directory", "error");
  const file = fsState.nodes.find(n => n.type === TYPE.FILE && n.name === fileName);
  if (!file) return showToast(`File "${fileName}" not found`, "error");
  const dir = fsState.nodes.find(n => [TYPE.DIR, TYPE.ROOT].includes(n.type) && n.name === dirName);
  if (!dir) return showToast(`Directory "${dirName}" not found`, "error");
  if (fsState.edges.find(e => e.from === dir.id && e.to === file.id))
    return showToast(`"${dirName}" already has a link to "${fileName}"`, "warn");
  addEdge(dir.id, file.id, "link");
  file.highlight = 1;
  const refCount = parentsOf(file.id).length;
  logTrace(`Hard link created: "${dirName}" → "${fileName}". Reference count = ${refCount}.`, "purple");
...
function opDagUnlink() {
  ...
  if (!fileName || !dirName) return showToast("Fill in file and directory names", "error");
  const file = fsState.nodes.find(n => n.type === TYPE.FILE && n.name === fileName);
  if (!file) return showToast(`File "${fileName}" not found`, "error");
  const dir = fsState.nodes.find(n => [TYPE.DIR, TYPE.ROOT].includes(n.type) && n.name === dirName);
  if (!dir) return showToast(`Directory "${dirName}" not found`, "error");
  ...
  const refCount = parentsOf(file.id).length;
  logTrace(`Removed link: "${dirName}" → "${fileName}". Reference count = ${refCount}.`, "warn");

  if (refCount === 0) {
    // Safe to delete
    fsState.nodes = fsState.nodes.filter(n => n.id !== file.id);
    fsState.edges = fsState.edges.filter(e => e.from !== file.id && e.to !== file.id);
    logTrace(`Reference count reached 0 — "${fileName}" deleted from disk.`, "error");
    showToast(`File deleted (ref=0)`, "info");
  } else {
    file.highlight = 0.5;
    logTrace(`File "${fileName}" still referenced by ${refCount} director${refCount === 1 ? "y" : "ies"}. Not deleted.`, "success");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  canvas.addEventListener("mousedown", e => {
    ...
    dragNode = nodeAt(x, y);
    if (dragNode) { isDragging = true; dragOffX = dragNode.x - x; dragOffY = dragNode.y - y; }
  });
